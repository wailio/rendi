...
export default function Categories() {
  return (
    <section className="relative py-4 md:py-8 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-[#123b32] via-[#0b2d25] to-[#050907]"></div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-3 md:px-6">
        <div className="mb-4 md:mb-6 relative inline-block w-full">
          ...
          <div className="h-0.5 md:h-1 w-12 md:w-20 mx-auto bg-gradient-to-r from-transparent via-[#6f9f87] to-transparent"></div>
        </div>
            ...
            return (
              ...
              <Link href={cat.href} className="flex justify-center shrink-0">
                <div className="flex flex-col items-center gap-1.5 md:gap-2 p-2 md:p-3 rounded-md hover:shadow-lg transition-all duration-300 cursor-pointer group hover:scale-105 w-14 md:w-20 h-20 md:h-20 justify-center border border-white/30 hover:border-white/60 bg-white/5 hover:bg-white/10">
                  ...
                  <div className="relative w-8 md:w-12 h-8 md:h-12 rounded-sm border-1.5 border-white flex items-center justify-center group-hover:border-[#6f9f87] group-hover:bg-white/10 transition-all duration-300">
                    <Icon className="w-5 md:w-6 h-5 md:h-6 text-white group-hover:text-[#6f9f87] transition-colors duration-300" />
                  </div>
                  
                  {/* Label - Extra small on mobile */}
                  <p className="text-[9px] md:text-xs font-semibold text-white text-center line-clamp-1 group-hover:text-[#6f9f87] transition-colors duration-300">{cat.label}</p>
                </div>
              </Link>
              </Reveal>
            )
